...
当前开发版本为 **v0.4.4**，处于 **Phase 2：有限一轮上下文与引用一致性收口完成**。

v0.3.2 保持不可变并继续作为 Phase 2 起点。v0.4.0 已建立失败归因、namespace 策略、独立阈值、事实证据门控和候选内轻量排序；v0.4.1 加入严格 calibration/holdout 协议和冻结匿名留出集；v0.4.2 固定真实 DeepSeek 验收的调用、token 和合成证据边界；v0.4.3 完成真实 `deepseek-v4-flash` 四场景验收；v0.4.4 固定当前轮 namespace 优先、兼容指代才继承上一问、每轮重新检索和只接受本轮引用。当前仍没有证据引入混合检索或第三方重排组件。
...
v0.4.4 默认离线测试为 **293 passed、2 skipped**；显式真实本地 Embedding 验收另有 **1 passed**，45-case 真实三源回归也已独立通过。原固定集保持 Router 100%、Hit@1 85.71%、Hit@5 100%、MRR 0.916667、硬负例拒绝 100%、跨 namespace 拒绝 100%、正负例 Agent 边界 100% 和引用完整性 100%。新增 8-case 匿名有限上下文矩阵和 Agent 对抗测试覆盖三域引用、跨域覆盖、词法硬负例、跨轮隐私外带与旧引用拒绝。schema v3 匿名集合的 calibration 和 holdout 均独立达到全部门槛；受控真实 LLM 用例仍默认跳过，只能在用户逐项确认后显式开启。v0.4.3 已获批的四场景远端验收保持有效，本轮未修改提示词或远端客户端，也没有新增真实调用。
