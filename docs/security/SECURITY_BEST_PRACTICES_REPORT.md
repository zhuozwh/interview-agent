...
- 影响：拥有相同 Windows 账户文件权限的进程可以读取或篡改 Chroma 正文、向量、SQLite 元数据，以及 v0.5.0 起受控保存的聊天问题和回答。
- 现有控制：索引与审计表仍不保存问题或回答正文；聊天表独立、有限保留、可逐会话/全部删除，且不保存检索证据、面试记录、密钥或内部片段标识；Chroma 关闭匿名遥测（`src/interview_agent/storage/chroma.py:58-65`）；运行时目录被 Git 忽略。
- 建议：若设备存在多账户共享或备份外发，关闭聊天历史或使用系统磁盘加密，并重新评估专用目录 ACL 和加密备份。
...
- SQLite 查询参数化；追踪表不保存问题、面试记录、证据或回答正文，聊天正文仅进入 v0.5.0 独立有限历史表。
...
- 本地聊天页使用随包静态资源、同源 CSP 和 `textContent`，不使用 CDN、浏览器持久化或 HTML 注入渲染。
- 历史正文成为显式敏感资产；默认 30 天、100 会话、每会话 100 轮，记录/读取时清理，并提供逐会话和全部删除。当前不做应用层加密，继续依赖单用户 Windows 账户与磁盘权限；该风险纳入 SEC-006。
- 启停使用 Git 忽略 PID 文件中的随机令牌。停止入口不进入 OpenAPI，错误令牌统一 404；脚本同时核验真实 PID、操作系统进程映像和启动时间。令牌被同账户进程读取最多导致可恢复的服务中断，不扩大 Vault 或密钥读取能力。
- v0.5.0 没有改变 Agent、Tool、Retrieval、提示词或 LLM 输出校验，也没有进行新的真实远端调用。完整结果见 `docs/acceptance/V0.5.0_LOCAL_USABILITY.md`。

## v0.5.2 展示与错误边界增补

- 回答只支持粗体、列表和行内代码三种有限格式；解析器输出结构化 token，浏览器只用 `createElement`、`createTextNode` 和 `textContent` 建立 DOM。HTML、Markdown 链接、脚本 URL 和伪造引用不会变成可执行元素。
- `/ask` 与聊天历史不再信任 `AgentError.message`。错误码先经过小写标识格式白名单，错误正文统一替换为按状态选择的固定公开文案；供应方异常、本机路径或密钥片段不能进入 HTTP 响应或 SQLite 聊天表。
- 重试没有定时器或自动循环；只有固定可重试错误显示按钮，用户确认“重新检索、可能产生一次调用和费用”后才经唯一 `/ask` 提交入口执行。
- 以上控制使用合成恶意 HTML、脚本 URL、路径、密钥片段和错误码完成离线对抗测试，没有发起新的真实远端 LLM 调用。完整证据见 `docs/acceptance/V0.5.2_DIAGNOSTIC_USABILITY.md`。

## v0.5.3 本地证据核验边界增补

- 证据接口只接受规范的 `session_id`、`trace_id` 和 `citation_id`，namespace、相对路径与行号必须来自服务端已保存历史；历史关闭、保存失败或记录不存在时不能退化为直接文件读取。
- 当前 Markdown 读取再次规范化固定数据源根、允许目录、真实文件路径、`.md` 后缀、UTF-8、文件大小和行号范围；目录穿越、绝对路径、反斜杠、驱动器、未知 namespace、符号链接越界和失效文件均失败关闭。
- 证据正文只在请求时有界读取，以 `textContent` 显示并返回 `no-store`；不增加 SQLite schema，不写浏览器持久化，也不回显绝对路径或底层文件异常。
- 面板明确区分“当前只读文件”与“回答时快照”，并说明检索分数只表示相关性排序、不是正确率，避免把可点击引用误当成语义蕴含证明。
- 合成对抗测试覆盖伪造身份、恶意 HTML/脚本正文、正文不落库、历史关闭、来源漂移、超大文件、非法 UTF-8、导入顺序和审计同时间戳；没有读取真实 Vault、请求 `/ask` 或发起远端调用。完整证据见 `docs/acceptance/V0.5.3_EVIDENCE_VERIFICATION.md`。

## v0.5.4 外部证据边界增补

- 默认应用装配没有真实外部搜索提供方；预览完全在本地执行，未配置时稳定返回不可用状态。真实 provider、endpoint、密钥、网络客户端、自动重试、抓取器和外部内容 LLM 合成都不在本版本中。
- 只有普通知识问题可以进入外部预览。项目、简历、面试记录、第一人称个人上下文、本地资料指代、明显批量外带、联系方式、本机路径和常见凭据形态均失败关闭；发生脱敏时只展示替换后的安全预览，不静默发送。
- 提供方存在时，浏览器在唯一搜索请求前展示提供方、实际查询和最多来源数，并用原生确认框取得用户同意。服务端重新计算策略并要求确认文本精确匹配，提供方接口规定一次调用且不得内部自动重试。
- 提供方候选按不可信数据处理：数量和文本有界，只接受公共 HTTPS 域名，拒绝凭据、敏感查询参数、自定义端口、所有字面 IP、本机/内网域名、控制字符和重复 URL。标题、摘要和类型只通过 `textContent` 显示，类型明确写成“提供方标记、来源未核验”。
- 外部查询和结果返回 `no-store`，只保存在当前页面对象，不进入 SQLite、浏览器持久化、`/ask`、本地引用或 LLM。切换/删除/新建会话会使在途结果失效，刷新即清空。
- 当前残余风险：个人信息与密钥识别不是完整 DLP；未来真实适配器仍需独立解决 DNS 重绑定、重定向、代理、超时、响应字节、供应方日志/保留、域名信誉和费用治理。接入前必须重新批准 provider、endpoint、次数、token/费用和允许外发范围。完整证据见 `docs/acceptance/V0.5.4_EXTERNAL_EVIDENCE_FOUNDATION.md`。
